...
grandTarget <- allData %>%
  filter( Event == "Search" & LatStim == "Target" & Session == "Day1" & searchRej == 0) %>%
...
grandDist <- allData %>%
  filter( Event == "Search" & LatStim == "Distractor" & Session == "Day1" & searchRej == 0) %>%
  mutate(sample = sample*srateMultiplier-baseline) %>%
  filter(sample > 200) %>%
  group_by(sample,Contra) %>%
  summarise(mean = mean(voltage)) %>%
...
grandTarget <- allData %>%
  ...
  spread(Contra, mean) %>% 
  mutate(diff = Contralateral - Ipsilateral)
DTNtStart = grandTarget$sample[grandTarget$diff == min(grandTarget$diff)]-winSize/2
DTNtEnd = grandTarget$sample[grandTarget$diff == min(grandTarget$diff)]+winSize/2
...
allData %>%
    ...
    theme_apa()
ggsave("Figures/SrchInitERPs.jpg", dpi = 300, width = 5, height = 3, units = c("in"))
```
...
search.Extd.TO <- allData %>%
  mutate(sample = sample*srateMultiplier-baseline) %>%
  filter(GroupP == "Unchanged", Event == "Search" & Stimulus == "Target (None mid)" & searchRej == 0 & sample>NtStart & sample < NtEnd) %>%
...
search.Extd.TD <- allData %>%
  mutate(sample = sample*srateMultiplier-baseline) %>%
  filter(GroupP == "Unchanged", Event == "Search" & Stimulus == "Target (Distractor mid)" & searchRej == 0 & sample>NtStart & sample < NtEnd) %>%
  group_by(Group1,Session,Subject,Contra) %>%
  summarise(mV = mean(voltage)) 
...
search.Extd.DTnt <- allData %>%
  mutate(sample = sample*srateMultiplier-baseline) %>%
  filter(GroupP == "Unchanged", Event == "Search" & Stimulus == "Distractor (Target mid)" & searchRej == 0 & sample>NtStart & sample < NtEnd) %>%
  group_by(Group1,Session,Subject,Contra) %>%
  summarise(mV = mean(voltage)) 
...
search.Extd.DTpd <- allData %>%
  mutate(sample = sample*srateMultiplier-baseline) %>%
  filter(GroupP == "Unchanged", Event == "Search" & Stimulus == "Distractor (Target mid)" & searchRej == 0 & sample>PdStart & sample < PdEnd) %>%
  group_by(Group1,Session,Subject,Contra) %>%
  summarise(mV = mean(voltage)) 
...
allData %>%
    ...
    geom_vline(xintercept = 0,aes(colour= "gray") ) +
    geom_hline(yintercept = 0,aes(colour= "gray") ) +
    geom_line( aes(colour = interaction(Group1,Session)),size=1) +
    #geom_line( aes(colour = Group1),size=1) +
    #geom_line(data = . %>% filter(Session == "Day3"), aes(colour = Group),linetype="dashed",size=3) +
    scale_color_manual(values=c("seagreen3", "gray52","seagreen3", "gray52"))+
    scale_x_continuous(name ="Latency (ms)", expand = c(0, 0), limits=c(-200, 500), breaks=seq(-200,500,200)) +
    scale_y_reverse(name =expression(paste("Amplitude (",mu,"v)")), expand = c(0, 0)) +
    facet_grid(.~Stimulus, labeller = labeller(Stimulus = c("Target (None mid)" = "Target Only", "Target (Distractor mid)" = "Target","Distractor (Target mid)" = "Distractor"))) +
    theme_apa()
ggsave("Figures/SrchExtdERPs.jpg", dpi = 300, width = 5, height = 3, units = c("in"))
```
...
search.Retr.TO <- allData %>%
  mutate(sample = sample*srateMultiplier-baseline) %>%
  filter(Session == "Day3", Event == "Search" & Stimulus == "Target (None mid)" & searchRej == 0 & sample>NtStart & sample < NtEnd) %>%
  group_by(Group1,GroupP,Subject,Contra) %>%
  summarise(mV = mean(voltage)) 
...
search.Retr.TD <- allData %>%
  mutate(sample = sample*srateMultiplier-baseline) %>%
  filter(Session == "Day3", Event == "Search" & Stimulus == "Target (Distractor mid)" & searchRej == 0 & sample>NtStart & sample < NtEnd) %>%
  group_by(Group1,GroupP,Subject,Contra) %>%
  summarise(mV = mean(voltage)) 
...
search.Retr.DTnt <- allData %>%
  mutate(sample = sample*srateMultiplier-baseline) %>%
  filter(Session == "Day3", Event == "Search" & Stimulus == "Distractor (Target mid)" & searchRej == 0 & sample>NtStart & sample < NtEnd) %>%
  group_by(Group1,GroupP,Subject,Contra) %>%
  summarise(mV = mean(voltage)) 
...
search.Retr.DTpd <- allData %>%
  mutate(sample = sample*srateMultiplier-baseline) %>%
  filter(Session == "Day3", Event == "Search" & Stimulus == "Distractor (Target mid)" & searchRej == 0 & sample>PdStart & sample < PdEnd) %>%
  group_by(Group1,GroupP,Subject,Contra) %>%
  summarise(mV = mean(voltage)) 
...
```{r saveSrchExtdERPData}
save(search.Extd.TO.results, search.Extd.TD.results, search.Extd.DTnt.results, search.Extd.DTpd.results, file = "DataOutput/SrchRetrERPANOVAs.RData")
```

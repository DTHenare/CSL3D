...
allData %>%
  ...
  mutate(diff = Contralateral - Ipsilateral) %>%
  ggplot(., aes(sample,diff)) +
    geom_rect(aes(xmin = NtStart, xmax = NtEnd, ymin = -Inf, ymax = 0), fill = "mistyrose1") + 
    geom_rect(aes(xmin = PdStart, xmax = PdEnd, ymin = 0, ymax = Inf), fill = "mistyrose1") +
    geom_line(aes(colour = Group1),size=3) +
    scale_colour_brewer(palette = "Set1") +
    scale_x_continuous(name ="Latency (ms)", expand = c(0, 0)) +
    scale_y_reverse(name =expression(paste("Amplitude (",mu,"v)")), expand = c(0, 0)) +
...
allData %>%
  ...
  mutate(diff = Contralateral - Ipsilateral) %>%
  ggplot(., aes(sample,diff)) +
      geom_rect(xmin = NtStart, xmax = NtEnd, ymin = Inf, ymax = 0, fill = "mistyrose1") +
    geom_rect(xmin = PdStart, xmax = PdEnd, ymin = -Inf, ymax = 0, fill = "mistyrose1") +
    geom_line(aes(colour = Session),size=3) +
    scale_colour_brewer(palette = "Set1") +
    scale_x_continuous(name ="Latency (ms)", expand = c(0, 0)) +
    scale_y_reverse(name =expression(paste("Amplitude (",mu,"v)")), expand = c(0, 0)) +
    ...
    theme_apa() +
    theme(panel.spacing.y = unit(10, "lines"), text= element_text(size=50))
ggsave("searchAll.pdf", width = 40, height = 30)
